...
export function NeonSetup({ onComplete, onSkip }: NeonSetupProps) {
	...
	const { data: orgs } = electronTrpc.atlas.neon.listOrganizations.useQuery(
		undefined,
		{
	...
	const saveTokenMutation = electronTrpc.atlas.neon.saveToken.useMutation({
		onSuccess: () => {
			refetchStatus();
			setStep("org");
		},
	...
	}, [status?.connected, orgs, step, onComplete]);
	...
	return (
		<div className="space-y-4 p-4 rounded-lg border border-border">
			...
			{orgs && orgs.length > 0 ? (
				...
				<p className="text-xs text-muted-foreground">조직을 불러오는 중...</p>
			)}
