...
const SIGNING_SECRET =
	args["signing-secret"] || process.env.LINEAR_WEBHOOK_SECRET || "";
...
async function handleLinearWebhook(rawBody, headers) {
	...
	if (type !== "Issue") {
		...
		return {
			status: 200,
			body: { ok: true, skipped: true, reason: `type ${type} not handled` },
		};
	}
	...
	if (stateName !== "In Progress") {
		...
		return {
			...
			body: {
				ok: true,
				skipped: true,
				reason: `state "${stateName}" is not In Progress`,
			},
		};
	}
	...
	for (const label of labels) {
		...
		if (inst) {
			...
			console.log(
				`  Label "${labelName}" → ${inst.name} (API ${inst.apiPort})`,
			);
		}
	}
	...
	if (targetInstances.length === 0) {
		...
		return {
			status: 200,
			body: { ok: true, skipped: true, reason: "no matching labels" },
		};
	}
