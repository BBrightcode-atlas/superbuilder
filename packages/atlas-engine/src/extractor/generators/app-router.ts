...
/**
 * packages/features/app-router.ts 재생성
 *
...
export function generateAppRouter(
  ...
  const filePath = join(targetPath, "packages/features/app-router.ts");
  ...
  for (const line of lines) {
    // Feature router import 라인 감지: import { xxxRouter } from "./yyy"
    // Core import (router from "@repo/core/trpc") 는 유지
    ...
    // 그 외 라인은 항상 유지 (boilerplate, comments, type export)
    result.push(line);
  }
