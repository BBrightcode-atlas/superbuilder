...
export async function superbuilderFeatureDevPipeline(
	...
	if (!cb?.onGenerate) {
		throw new Error("onGenerate 콜백이 필요합니다. AI 작업은 호출자가 수행해야 합니다.");
	}
	...
	cb?.onStep?.("generateSpec", "done", `Spec 완료 (feature: ${result.featureName})`);
	...
	if (!opts.skipVerify) {
		...
		try {
			...
			cb?.onStep?.("verify", "error", e instanceof Error ? e.message : String(e));
		}
	} else {
	...
	if (!opts.skipRegister) {
		...
		try {
			...
			const { stdout: prOutput } = await execFile(
				"gh",
				["pr", "create", "--base", "develop", "--title", `feat: add ${result.featureName} feature`, "--body", `Auto-generated by superbuilder-feature-dev`],
				{ cwd: worktreePath, timeout: 60_000 },
			);
			result.prUrl = prOutput.trim();
			cb?.onStep?.("register", "done", `PR: ${result.prUrl}`);
		} catch (e) {
			cb?.onStep?.("register", "error", e instanceof Error ? e.message : String(e));
		}
	} else {
	...
	if (result.prUrl) {
		const regDecision = await requestApproval("registration", undefined, `PR: ${result.prUrl}`);
	...
	if (result.prUrl) {
		...
		try {
			await execFile("gh", ["pr", "merge", result.prUrl, "--merge", "--delete-branch"], {
				cwd: worktreePath, timeout: 60_000,
			});
			cb?.onStep?.("complete", "done", "머지 완료");
		} catch (e) {
			cb?.onStep?.("complete", "error", e instanceof Error ? e.message : String(e));
		}
	}
...
async function cleanupWorktree(
	...
	try {
		...
		cb?.onStep?.("cleanup", "error", `수동 제거 필요: git worktree remove ${worktreePath} --force`);
	}
}
