...
export async function composePipeline(
	...
	cb?.onLog?.("피처 의존성 해석 중...");
	const manifest = await fetchRemoteManifest({
		repo: opts.boilerplateRepo,
	});
	const resolved = resolveFeatures(manifest, input.selected);
	cb?.onLog?.(
		`해석 완료: ${resolved.resolved.length}개 피처 (자동 포함 ${resolved.autoIncluded.length}개)`,
	);

	// ── Step 2: scaffold (FATAL) ─────────────────────────────────
	cb?.onStep?.("scaffold");
	cb?.onLog?.("프로젝트 스캐폴딩 중...");
	...
	cb?.onLog?.(
		`스캐폴드 완료: ${scaffoldResult.keptFeatures.length}개 피처 유지, ${scaffoldResult.removedFeatures.length}개 제거`,
	);
	...
	if (opts.neon) {
		...
		try {
			...
			cb?.onLog?.(`Neon DB 생성 완료: ${neonResult.projectId}`);
		} catch (e) {
			const msg = e instanceof Error ? e.message : String(e);
			cb?.onLog?.(`Neon DB 생성 실패 — DB 없이 계속 진행합니다: ${msg}`);
		}
	}
	...
	if (opts.github) {
		...
		try {
			...
			cb?.onLog?.(`GitHub 레포 생성 완료: ${githubResult.repoUrl}`);
		} catch (e) {
			const msg = e instanceof Error ? e.message : String(e);
			cb?.onLog?.(`GitHub 레포 생성 실패 — 계속 진행합니다: ${msg}`);
		}
	}
	...
	if (opts.vercel && githubResult) {
		...
		try {
			...
			cb?.onLog?.(`Vercel 배포 완료: ${vercelResult.deploymentUrl}`);
		} catch (e) {
			const msg = e instanceof Error ? e.message : String(e);
			cb?.onLog?.(`Vercel 배포 실패 — 계속 진행합니다: ${msg}`);
		}
	} else if (opts.vercel && !githubResult) {
		cb?.onLog?.("Vercel 배포 건너뜀 — GitHub 레포가 없습니다");
	}

	// ── Step 6: env (NON-FATAL) ──────────────────────────────────
	cb?.onStep?.("env");
	cb?.onLog?.(".env 파일 생성 중...");
	try {
		...
		cb?.onLog?.(".env 파일 생성 완료");
	} catch (e) {
		const msg = e instanceof Error ? e.message : String(e);
		cb?.onLog?.(`.env 파일 생성 실패: ${msg}`);
	}
	...
	if (opts.install) {
		...
		cb?.onLog?.("의존성 설치 중...");
		try {
			const installResult = await installFeatures({ projectDir });
			installed = installResult.installed;
			if (installResult.migrated) {
				cb?.onStep?.("migrate");
				cb?.onLog?.("DB 마이그레이션 완료");
			}
			cb?.onLog?.("의존성 설치 완료");
		} catch (e) {
			const msg = e instanceof Error ? e.message : String(e);
			cb?.onLog?.(`의존성 설치 실패: ${msg}`);
		}
	}

	// ── Step 8: seed (NON-FATAL, opt-in, requires install+neon) ──
	let seedResult: SeedResult | undefined;
	if (installed && neonResult && (opts.ownerEmail || opts.ownerPassword)) {
		cb?.onStep?.("seed");
		cb?.onLog?.("초기 데이터 시딩 중...");
		try {
			...
			cb?.onLog?.(`시딩 완료: ${seedResult.ownerEmail}`);
		} catch (e) {
			const msg = e instanceof Error ? e.message : String(e);
			cb?.onLog?.(`시딩 실패: ${msg}`);
		}
