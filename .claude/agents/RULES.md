...
에이전트는 재시작되면 이전 맥락을 모른다. 아래 3가지로 복구한다.

**1. .cos-resume.md (로컬 작업 노트)**

작업 중 의미 있는 진전이 있을 때마다 프로젝트 루트에 `.cos-resume.md`를 갱신한다:
...
**2. Linear 이슈 코멘트** — 착수/단계전환/완료를 코멘트로 남긴다 (위 규칙 참조).
